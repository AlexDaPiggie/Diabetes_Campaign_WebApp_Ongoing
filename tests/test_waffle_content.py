...
class WaffleContentTests(unittest.TestCase):
    ...
    def test_waffle_legend_labels(self):
        ...
        self.assertIn("Are undiagnosed", response.text)
        self.assertIn("115.2M", response.text)
        self.assertIn("Have prediabetes", response.text)
        self.assertIn("178.9M", response.text)
        self.assertIn("Are Healthy", response.text)

    def test_elderly_callout_presence(self):
        response = TestClient(app).get("/")
        self.assertIn("Up to <strong>52.1%</strong> adults <strong>over 64</strong> have <strong>prediabetes</strong>.", response.text)

    def test_waffle_source_attribution(self):
        response = TestClient(app).get("/")
        
        # Verify source text
        self.assertIn("US National Diabetes Statistics Report 2024", response.text)
